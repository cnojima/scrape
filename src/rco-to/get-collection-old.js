...
const getCollection = (page, options, config) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const isDone = async () => {
      ...
      if (books.length === 0 && cbzToDo === 0) {
        history(options, config);
        resolve(toNuke);
      } else {
        l.debug(`cbzToDo has ${cbzToDo} archives to build :: ${books.length} books to process`.cyan);
    ...
    if (books.length > 0) {
      while (books.length > 0) {
        ...
        if (!fs.existsSync(cbzDest) || options['force-archive'] === true) {
          ...
          // at book pages with all pages rendered inline
          await page.waitForNavigation({
            timeout: 30000,
